...
export function OfficialSimulation({ result, comparison, name, comparisonName, selected, hasStrategy, hasComparison, busy, pending, onRun, onExplain }: Props) {
  ...
  return <section className="official-section" aria-labelledby="official-title"><div className="comparison-heading"><h2 id="official-title"><CheckCircle2 size={16} />Official 2-Year Simulation</h2><span>2026 → 2028 · H = 8 quarters</span></div>
    <div className="official-actions"><p>Deterministic HackAlem model · official weights, implementation lags and synergies.</p><Button className="primary-action" disabled={busy || !hasStrategy} onClick={onRun}><Play size={14} />{pending === "simulation" ? "Simulating…" : result ? "Run again" : "Run Official 2-Year Simulation"}</Button></div>
    {!result ? <Card className="empty-state simulation-empty"><Gauge size={28} /><div><h3>{hasStrategy ? "Your strategy is ready for simulation" : "Build your strategy in the chat"}</h3><p>{hasStrategy ? `Run the official engine to calculate ${hasComparison ? "both strategies’ scores" : "your strategy’s score"} and district-level changes.` : "Choose five initiatives and their districts in the chat, then confirm your decisions. The total cost must stay within 100 units."}</p></div></Card> : <>
