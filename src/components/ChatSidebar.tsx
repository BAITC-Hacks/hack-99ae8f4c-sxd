...
import { ArrowUp, ArrowUpRight, Check, Crosshair, MapPin, Plus, ScanLine, Terminal, UserRound } from "lucide-react";
import { Button } from "./ui/button";

type Message = { role: "assistant" | "user"; text: string; prepared?: boolean };
type ChatSidebarProps = { busy?: boolean; canExplain?: boolean; strategyBuilder?: ReactNode; onGenerate?: (intent: string) => Promise<string>; onExplain?: (question: string) => Promise<string>; onReset?: () => void };
...
export function ChatSidebar({ busy = false, canExplain = false, strategyBuilder, onGenerate, onExplain, onReset }: ChatSidebarProps) {
  ...
  return <aside className="chat-sidebar" aria-label="FARSIGHT copilot">
    <div className="sidebar-brand"><div className="brand-line"><span className="brand-mark"><Crosshair size={24} strokeWidth={1.4} /></span><strong>FARSIGHT</strong><span className="version-label">BETA</span></div><p>Future Akim Strategy &amp;<br />Impact Governance Tool</p></div>
    <div className="sidebar-context"><span><MapPin size={13} />Astana, Kazakhstan</span><span className="context-year">{onGenerate ? "2026–2028" : "2050"}</span></div>
    <div className="copilot-heading"><span><Terminal size={15} />AI Copilot</span><Button variant="ghost" size="icon" disabled={busy} aria-label="Start a new strategy session" title="New conversation" onClick={() => { onReset?.(); setMode("strategy"); setMessages([initialMessages[0]]); setInput(""); setPreset(null); inputRef.current?.focus(); }}><Plus size={16} /></Button></div>
